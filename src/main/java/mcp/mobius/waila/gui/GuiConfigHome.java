...
public class GuiConfigHome extends Screen {
    ...
    @Override
    protected void init() {
        addSelectableChild(new ButtonWidget(width / 2 - 105, height / 2 - 10, 100, 20, new TranslatableText("gui.waila.waila_settings", Waila.NAME), w -> {
            client.openScreen(new GuiConfigWaila(GuiConfigHome.this));
        }));
        addSelectableChild(new ButtonWidget(width / 2 + 5, height / 2 - 10, 100, 20, new TranslatableText("gui.waila.plugin_settings"), w -> {
            client.openScreen(new GuiConfigPlugins(GuiConfigHome.this));
        }));
        addSelectableChild(new ButtonWidget(width / 2 - 50, height / 2 + 20, 100, 20, new TranslatableText("gui.done"), w -> {
            Waila.CONFIG.save();
